...
def load_data(traindata, testdata=False):
    """
    Loads data from testdata if set, otherwise from traindata.
    Important that the files are in a `data` directory
    """
    fname = testdata if testdata else traindata
    ...
    if not testdata:  # only FTRAIN has target columns
